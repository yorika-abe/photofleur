...
export default function LoginForm({ redirect, isAlreadyRegistered, noticeEmail, lineError, lineBlocked }) {
  ...
  function pushByRoles(roles, role) {
    if (redirect !== '/') { router.push(redirect); return }
    if (roles.includes('owner') || role === 'owner') router.push('/admin')
    else if (roles.includes('admin')) router.push('/admin')
    else if (roles.includes('model')) router.push('/model-portal')
    else if (roles.includes('staff')) router.push('/staff-portal')
    else router.push('/my')
    router.refresh()
  }
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      // TOKEN_REFRESHEDのみ対応（すでにログイン済みでページを開いた場合）
      // SIGNED_INはhandleLogin側でリダイレクト済みのため除外
      if (session && event === 'TOKEN_REFRESHED') {
        const { data: profile } = await supabase.from('user_profiles').select('roles, role').eq('id', session.user.id).single()
        const roles = profile?.roles?.length > 0 ? profile.roles : (profile?.role ? [profile.role] : [])
        pushByRoles(roles, profile?.role)
      }
    })
    ...
    <div style={{ maxWidth: 440, margin: '60px auto', padding: '0 20px' }}>
      ...
      <a
        ...
        style={{
          ...
          background: lineLoading ? '#05a348' : '#06C755', color: '#fff', textDecoration: 'none',
          borderRadius: 10, padding: '14px', fontSize: 16, fontWeight: 700,
          marginBottom: 16, boxShadow: '0 2px 8px rgba(6,199,85,0.3)',
          opacity: lineLoading ? 0.85 : 1, pointerEvents: lineLoading ? 'none' : 'auto',
        }}
      >
        ...
        {lineLoading ? 'LINEへ移動中...' : 'LINEでログイン'}
      </a>
