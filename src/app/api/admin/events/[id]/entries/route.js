...
export async function POST(req, { params }) {
  ...
  if (body.action === 'auto_add') {
    ...
    for (const shift of toAdd) {
      ...
      const targetSlots = submittedStarts.length > 0 ? allSlots.filter(s => submittedStarts.includes(s.start)) : allSlots.filter(s => s.order !== 0)
  ...
  if (body.action === 'add_model') {
    ...
    const { data: shift } = await supabase.from('model_shifts').select('available_slots').eq('model_id', modelId).eq('event_date', event.event_date).order('created_at', { ascending: false }).limit(1).single()
    const allSlots = event.event_type === 'studio' ? STUDIO_SLOTS : STREET_SLOTS
    const submittedStarts = (shift?.available_slots || []).map(s => s.start)
    const targetSlots = submittedStarts.length > 0 ? allSlots.filter(s => submittedStarts.includes(s.start)) : allSlots.filter(s => s.order !== 0)
