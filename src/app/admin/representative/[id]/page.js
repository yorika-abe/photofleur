...
export default function AdminRepresentativeEditPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/representatives/${id}`)
      .then(r => r.json())
      .then(data => {
        setForm({
          photo: data.photo || '',
  ...
  async function save() {
    setSaving(true)
    try {
      const res = await fetch(`/api/admin/representatives/${id}`, {
        method: 'PUT',
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', padding: '40px 20px' }}>
      ...
      {cropSrc && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <Cropper image={cropSrc} crop={crop} zoom={zoom} aspect={4 / 5}
      ...
      <Link href="/admin/representative" style={{ color: '#1a3560', fontSize: 13, textDecoration: 'none' }}>← 代表メッセージ一覧</Link>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a3560', margin: '16px 0 28px' }}>代表メッセージ編集</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              {loaded ? (
                ...
                <div style={{ border: '1px solid #ddd', borderRadius: 10, minHeight: 200, padding: 16, color: '#aaa', fontSize: 13 }}>読み込み中...</div>
              )}
        ...
        <div style={{ background: '#fff', border: '1px solid #d6ecf5', borderRadius: 14, padding: '24px' }}>
          ...
          <p style={{ fontSize: 12, color: '#aaa', marginBottom: 16, marginTop: 0 }}>モデルとして活動している場合、モデルページへのリンクを表示できます</p>
      ...
      <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <Link href="/admin/representative" style={{ fontSize: 13, color: '#888', textDecoration: 'none' }}>一覧に戻る</Link>
      </div>
    </div>
  )
}
