...
function ResizableImage({ data, onResize }) {
  ...
  const startDrag = useCallback((e, dir) => {
    ...
    const onMove = (ev) => {
      ...
      if (dir === 'corner') {
        onResize({ width: `${Math.max(60, startW + dx)}px`, height: `${Math.max(40, startH + dy)}px` })
      } else if (dir === 'right') {
        onResize({ width: `${Math.max(60, startW + dx)}px` })
      } else if (dir === 'bottom') {
        onResize({ height: `${Math.max(40, startH + dy)}px` })
      }
    }
    const onUp = () => {
...
export default function NewsletterPage() {
  ...
  const [dropTargetId, setDropTargetId] = useState(null) // id of block to insert BEFORE (null = end)
  ...
  function handleDragOver(e, targetId) {
    e.preventDefault()
    e.stopPropagation()
    if (dragBlockId.current == null || dragBlockId.current === targetId) return
    setDropTargetId(targetId)
  }

  function handleDragOverEnd(e) {
    e.preventDefault()
    setDropTargetId(null) // drop at end
  }

  function handleDrop(insertBeforeId) {
    const srcId = dragBlockId.current
    if (srcId == null) return
    setBlocks(prev => {
      const src = prev.find(b => b.id === srcId)
      if (!src) return prev
      const without = prev.filter(b => b.id !== srcId)
      if (insertBeforeId === null) return [...without, src]
      const idx = without.findIndex(b => b.id === insertBeforeId)
          ...
          <div style={{ maxWidth: 600, margin: '0 auto', background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', borderRadius: 8, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '24px 32px' }} onClick={e => e.stopPropagation()}
                ...
                return (
                  <div key={block.id}>
                    ...
                    <div
                      ...
                      style={{ height: isDropTarget ? 6 : 4, background: isDropTarget ? '#5bbfd6' : 'transparent', borderRadius: 3, margin: '2px 0', transition: 'all 0.1s' }}
                    />

                    {/* Block row: handle + content + delete */}
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 4, position: 'relative' }}>
                      ...
                      <div
                        ...
                        title="ドラッグして並び替え"
                        style={{ paddingTop: 10, cursor: 'grab', color: '#ccc', fontSize: 16, flexShrink: 0, userSelect: 'none', lineHeight: 1 }}>
                        ⠿
                      </div>
                      ...
                      <div
                        ...
                        style={{ flex: 1, minWidth: 0, outline: isSelected ? '2px solid #1a3560' : '2px solid transparent', borderRadius: 4, padding: 8, background: isSelected ? '#f0f5ff' : 'transparent', cursor: 'pointer' }}>
                        {isSelected && block.type === 'image'
                          ? <ResizableImage data={block.data} onResize={updates => updateBlock(block.id, { ...block.data, ...updates })} />
                      ...
                      {isSelected && (
                        <button
                          ...
                          style={{ ...ctrlBtn, background: '#e53935', flexShrink: 0, marginTop: 8 }}>
                          ✕
                        </button>
                      )}
                    </div>
              ...
              <div
                ...
                style={{ height: dropTargetId === null && dragBlockId.current != null ? 6 : 4, background: dropTargetId === null && dragBlockId.current != null ? '#5bbfd6' : 'transparent', borderRadius: 3, margin: '2px 0', transition: 'all 0.1s', minHeight: 20 }}
              />
            </div>
